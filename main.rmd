...
```{r,eval=TRUE, echo=TRUE, include=TRUE}
# Given a number of runs of Kruskal's Count, finds the average probability of success
# M = Number of Runs
# values = Facecard Values
calc_prob_success_human <- function(M, value) {
  runs = rep(0, M)
...
face_val_prob_unif = rep(0, 10)
face_val_prob_human = rep(0, 10)

# Probability of Success Over 1-10 Values of Face Card with Uniform and Human Distribution
for(i in 1:10){
  face_val_prob_unif[i] = calc_prob_success_uniform(1000, face_val[i])
  face_val_prob_human[i] = calc_prob_success_human(1000, face_val[i])

}

print(cbind(face_val, face_val_prob_unif, face_val_prob_human))
