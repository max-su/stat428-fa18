...
(Explain here what this does and why its an example of a random number generation. (Kasi?))
...
(Explain here what this does and why its example of sampling (Max?))
```{r,eval=TRUE, echo=TRUE, include=TRUE}
...
(Brief description of what it does (can probably use comments below...))
```{r,eval=TRUE, echo=TRUE, include=TRUE}
...
calc_prob_success_human <- function(M, V) {
...
for(i in 1:10){
  ...
  Human_Prob[i] = calc_prob_success_human(1000, Face_Card_Value[i])
}
```
...
## Average Card Value Optimization  
...
# Average Card Deck Size Optimization
...
plot(fx, fy)
```
We see that probability function given a fixed mean card value is increasingly asymptotic 
...
# Two-Dimensional Optimization for Average Value and Sample Size
(Brief description of function (Adam?))(rename header if it doesnt make sense)
```{r}
